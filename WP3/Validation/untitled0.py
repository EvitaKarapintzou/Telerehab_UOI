# -*- coding: utf-8 -*-
"""
Created on Wed Jun  5 17:02:05 2024
...
# # Calculate metrics for each trial (e.g., mean and std of yaw)
# mean_yaw_trial1 = imu_data_trial1['yaw'].mean()
# std_yaw_trial1 = imu_data_trial1['yaw'].std()

# mean_yaw_trial2 = imu_data_trial2['yaw'].mean()
# std_yaw_trial2 = imu_data_trial2['yaw'].std()

# mean_yaw_trial3 = imu_data_trial3['yaw'].mean()
# std_yaw_trial3 = imu_data_trial3['yaw'].std()
...
# # Calculate ICC
# yaw_data = np.array([imu_data_trial1['yaw'], imu_data_trial2['yaw'], imu_data_trial3['yaw']])
# mean_yaw = np.mean(yaw_data, axis=0)
# sum_squares_total = np.sum((yaw_data - mean_yaw) ** 2)
# sum_squares_within = np.sum((yaw_data - yaw_data.mean(axis=1, keepdims=True)) ** 2)
# icc = 1 - (sum_squares_within / sum_squares_total)
# print(f'ICC for Yaw: {icc}')
...
plt.title('Yaw Data from Multiple Trials')
plt.xlabel('Time')
