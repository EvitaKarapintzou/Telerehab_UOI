...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if len(Limu1) > 0 and len(Limu2) > 0:
        returnedJson = getMetricsSittingOld01(Limu1, Limu2, False) 
        return returnedJson

def getMetricsSittingOld01(Limu1, Limu2, plotdiagrams):
...
#    IMU2 BACK
    ...
    # mean_range = np.mean(filtered_ranges) if filtered_ranges else -1
    # std_range = np.std(filtered_ranges, ddof=1) if filtered_ranges else -1

    # mean_duration = np.mean(movement_durations) if movement_durations else -1
    # std_duration = np.std(movement_durations, ddof=1) if movement_durations else -1   
    ...
    filename = f"{datetime_string}_VOR_Horizontal_metrics.txt"
...
def save_metrics_to_txt(metrics, file_path):
    ...
    sub_directory = "VOR_Horizontal Metrics Data"
