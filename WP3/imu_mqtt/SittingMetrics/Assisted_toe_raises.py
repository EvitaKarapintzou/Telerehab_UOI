...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if(len(Limu1) > 0):
        returnedJson = getMetricsSittingNew02(Limu1, False) 
        return returnedJson

def getMetricsSittingNew02(Limu1, plotdiagrams):
    ...
    X_filtered = butter_lowpass_filter(quaternions_df['X(number)'], cutoff, fs, order=5)
    Z_filtered = butter_lowpass_filter(quaternions_df['Z (number)'], cutoff, fs, order=5)
    ...
    if (plotdiagrams):
        ...
        plt.plot(quaternions_df.index, movement_magnitude, label='Movement Magnitude', linewidth=2)
        plt.xlabel('Timestamp')
    ...
    if (plotdiagrams):
        plt.figure(figsize=(12, 6))
        plt.plot(X_filtered, label='X_filtered', linewidth=1)
    ...
    for valley, peak in movement_pairs:
        yaw_range = abs(X_filtered[peak] - X_filtered[valley])
        movement_ranges_yaw.append(yaw_range)
    
        roll_range = abs(Z_filtered[peak] - Z_filtered[valley])
    ...
    significant_movements = [(pair, yaw, roll, np.sqrt(yaw**2 + roll**2)) for pair, yaw, roll in zip(movement_pairs, movement_ranges_yaw, movement_ranges_roll) if np.sqrt(yaw**2 + roll**2) >= 0.1]
