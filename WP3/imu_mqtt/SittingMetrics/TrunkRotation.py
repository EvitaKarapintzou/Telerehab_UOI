...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if(len(Limu1) > 0):
        returnedJson = getMetricsSittingNew01(Limu1, False) 
        return returnedJson

def getMetricsSittingNew01(Limu1, plotdiagrams):
    ...
    significant_movements = [(pair, yaw, roll, np.sqrt(yaw**2 + roll**2)) for pair, yaw, roll in zip(movement_pairs, movement_ranges_yaw, movement_ranges_roll) if np.sqrt(yaw**2 + roll**2) >= 1]
    ...
    filename = f"{datetime_string}_TrunkRotation_metrics.txt"
...
def save_metrics_to_txt(metrics, file_path):
    ...
    sub_directory = "TrunkRotation Metrics Data"
