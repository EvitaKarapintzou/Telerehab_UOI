...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if len(Limu1) > 0 and len(Limu2) > 0:
        returnedJson = getMetricsSittingNew05(Limu1, Limu2, False) 
        return returnedJson

def getMetricsSittingNew05(Limu1, Limu2, plotdiagrams):
    ...
    while current_time + interval_length <= end_time:
        ...
        metrics = getMetricsSittingNew05(interval_data1, interval_data2)  # Placeholder for the real calculation function
    ...
    # total_distance1 = np.trapz(Limu1['Velocity'], dx=1/30)  # Assuming data is sampled at 30 Hz
...
#=============IMU2-Left Leg====================#
    ...
    # total_distance2 = np.trapz(Limu2['Velocity'], dx=1/30)  # Assuming data is sampled at 30 Hz
    ...
    metrics_data = {
        ...
        "total_metrics": {
            ...
            "symmetry": float(symmetry_index),
            "acceleration1": float(peak_acceleration2)
             }
    ...
    filename = f"{datetime_string}_SitToStand_metrics.txt"
