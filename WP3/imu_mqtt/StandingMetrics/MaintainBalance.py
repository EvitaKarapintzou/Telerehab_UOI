...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if(len(Limu1) > 0):
        returnedJson = getMetricsStandingNew01(Limu1, False) 
        return returnedJson

def getMetricsStandingNew01(Limu1, plotdiagrams):
    ...
    # filtered_gyro_y = butter_lowpass_filter(df_Limu1['gyro_y'], 0.6, 30)
    # # Detect significant deviations (beyond ±4 degrees threshold)
    # deviations = np.abs(filtered_gyro_y) > 4
    # return np.any(deviations)
    # # Calculate each metric
    # posture_deviation = calculate_posture_deviation(df_Limu1)
    # cg_displacement = calculate_cg_displacement(imu_data)  # Could use both imu and pressure data
    # balance_loss = calculate_balance_loss(imu_data)

    # # Determine if feedback is needed
    # if posture_deviation > 4 or cg_displacement > 4:
    #     give_feedback("Adjust posture and center of gravity.")
    # if balance_loss:
    ...
    filename = f"{datetime_string}_MaintainBalance_metrics.txt"
...
def save_metrics_to_txt(metrics, file_path):
    ...
    sub_directory = "MaintainBalance Metrics Data"
