...
# imu_data_sensor_head = pd.read_csv(imu_data_path_sensor_head)
# imu_data_sensor_back = pd.read_csv(imu_data_path_sensor_back)
...
# quaternions = imu_data_sensor_back[['W(number)', 'X(number)', 'Y (number)', 'Z (number)']].to_numpy()
...
# maxima_indices_back = find_peaks(imu_data_sensor_back['Y (number)'], height=-0.6)[0]
# # Invert the signal to find the min peaks and then filter them with the condition that they are less than -0.7
# min_peaks_back, _ = find_peaks(-imu_data_sensor_back['Y (number)'])
# filtered_min_peaks_back = min_peaks_back[imu_data_sensor_back['Y (number)'][min_peaks_back] < -0.7]
...
# filtered_maxima_indices_back = maxima_indices_back[imu_data_sensor_back['Y (number)'][maxima_indices_back] > -0.65]
...
# rom_back = np.ptp(imu_data_sensor_back['Y (number)'])  # Peak-to-peak (max-min) range

# # Calculate the speed/tempo of rotations
# # Assuming that `Elapsed(s)` is uniformly sampled
# time_diffs = np.diff(imu_data_sensor_back['Elapsed(s)'][maxima_indices_back])
# rotation_speed = 1 / time_diffs.mean()  # Average rotations per second

# # Assess the consistency
# peak_values_back = imu_data_sensor_back['Y (number)'][maxima_indices_back]
