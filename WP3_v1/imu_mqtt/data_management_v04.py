...
# Example function to simulate IMU data production and put it into the queue
def imu_data_producer(q):
    body_parts = ['HEAD', 'PELVIS', 'LEFTFOOT', 'RIGHTFOOT']
    
    for i in range(500):  # Simulate 500 data points
        for body_part in body_parts:
            # Simulate sensor data with random values (replace with real data in practice)
            data = {
                "deviceMacAddress": body_part,
                "timestamp": time.time(),
                "w": np.sin(i / 10) + np.random.random() * 0.1,
                "x": np.cos(i / 10) + np.random.random() * 0.1,
                "y": np.sin(i / 20) + np.random.random() * 0.1,
                "z": np.cos(i / 20) + np.random.random() * 0.1
            }
            json_data = json.dumps(data)
            q.put(json_data)  # Put simulated data in the queue
        
        time.sleep(0.1)  # Simulate delay between IMU data readings
...
lines = {}  # Store plot lines for each body part
...
def receive_imu_data(q, scheduleQueue, config_message, exercise):
    ...
    # Initialize a plot for each expected IMU sensor
    for ax, label in zip(axs, ['HEAD', 'PELVIS', 'LEFTFOOT', 'RIGHTFOOT']):
        ax.set_title(f"{label} IMU Data")
        ax.set_xlim(0, 100)
        ax.set_ylim(-1, 0)  # Assuming IMU data is between -2 and 2, adjust as necessary
        ax.grid(True)
