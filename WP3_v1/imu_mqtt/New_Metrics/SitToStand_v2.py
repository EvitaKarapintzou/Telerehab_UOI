...
def getMetricsSittingNew01(Limu1, Limu2, plotdiagrams=False):
    ...
    timestamps = pd.to_numeric(df_Limu2.index) / 1e6  # Convert to seconds for consistency
    ...
    peaks, _ = find_peaks(smoothed_signal, prominence=0.02)
    valleys, _ = find_peaks(-smoothed_signal, prominence=0.02)

    # Logic to detect movements
    final_refined_movements = []
    sorted_valleys = sorted(valleys)
    sorted_peaks = sorted(peaks)

    i = 0
    while i < len(sorted_valleys) - 1:
        current_valley_idx = sorted_valleys[i]
        pattern_indices = [current_valley_idx]
        try:
            # Look for the min -> max -> min -> max -> min -> max -> min pattern
            first_peak = next(idx for idx in sorted_peaks if idx > current_valley_idx)
            pattern_indices.append(first_peak)

            second_valley = next(idx for idx in sorted_valleys if idx > first_peak)
            pattern_indices.append(second_valley)

            second_peak = next(idx for idx in sorted_peaks if idx > second_valley)
            pattern_indices.append(second_peak)

            third_valley = next(idx for idx in sorted_valleys if idx > second_peak)
            pattern_indices.append(third_valley)

            third_peak = next(idx for idx in sorted_peaks if idx > third_valley)
            pattern_indices.append(third_peak)

            # Include the last valley explicitly
            final_valley_candidates = [idx for idx in sorted_valleys if idx > third_peak]
            final_valley = final_valley_candidates[0] if final_valley_candidates else sorted_valleys[-1]
            pattern_indices.append(final_valley)

            # Validate range of the movement
            movement_range = smoothed_signal.iloc[pattern_indices].max() - smoothed_signal.iloc[pattern_indices].min()
            if movement_range > 0.05:  # Apply range threshold
                final_refined_movements.append({
                    "start_time": timestamps.iloc[current_valley_idx],
                    "end_time": timestamps.iloc[final_valley],
                    "range_degrees": movement_range,
                    "duration": (timestamps.iloc[final_valley] - timestamps.iloc[current_valley_idx])  # Duration in seconds
                })

            i = sorted_valleys.index(final_valley)
    ...
    # Metrics calculation
    if final_refined_movements:
        durations = [move['duration'] for move in final_refined_movements]
        ranges = [move['range_degrees'] for move in final_refined_movements]
        exercise_duration_seconds = (timestamps.iloc[-1] - timestamps.iloc[0])  # Total duration in seconds

        metrics_data = {
            "number_of_movements": len(final_refined_movements),
            "pace_movements_per_second": len(final_refined_movements) / exercise_duration_seconds,
            ...
            "Exercise duration (seconds)": exercise_duration_seconds
        }
