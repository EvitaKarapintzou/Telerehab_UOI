...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if(len(Limu2) > 0):
        returnedJson = getMetricsStandingOld04(Limu2, False) 
        return returnedJson

def getMetricsStandingOld04(Limu2, plotdiagrams):
    ...
    significant_movements = [(pair, yaw, roll, np.sqrt(yaw**2 + roll**2)) for pair, yaw, roll in zip(movement_pairs, movement_ranges_yaw, movement_ranges_roll) if np.sqrt(yaw**2 + roll**2) >= 10]
