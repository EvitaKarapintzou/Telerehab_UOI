...
def getMetricsSittingNew01(Limu2, plotdiagrams):
    ...
    roll_filtered = butter_lowpass_filter(euler_df_degrees2['Roll (degrees)'], cutoff, fs, order=5)
    ...
    if (plotdiagrams):
        ...
        plt.plot(euler_df_degrees2.index, movement_magnitude, label='Movement Magnitude', linewidth=2)
        plt.xlabel('Timestamp')
    ...
    significant_movements = [(pair, yaw, roll, np.sqrt(yaw**2 + roll**2)) for pair, yaw, roll in zip(movement_pairs, movement_ranges_yaw, movement_ranges_roll) if np.sqrt(yaw**2 + roll**2) >= 15]
    ...
    filename = f"{datetime_string}_SeatedTrunkRotation_metrics.txt"
...
def save_metrics_to_txt(metrics, file_path):
    ...
    sub_directory = "SeatedTrunkRotation Metrics Data"
