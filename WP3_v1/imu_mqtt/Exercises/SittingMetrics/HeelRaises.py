...
def get_metrics(imu1,imu2,imu3,imu4, counter):
    ...
    if len(Limu1) > 0 and len(Limu2) > 0:
        returnedJson = getMetricsSittingNew03(Limu1, Limu2, True) 
        return returnedJson
...
# Calculate the magnitude of movement considering both yaw and roll
    ...
    if (plotdiagrams):
        plt.figure(figsize=(12, 6))
        plt.plot(movement_magnitude2, label='movement_magnitude2', linewidth=1)
...
#=============IMU2-Left Leg====================#
    ...
    for i, yaw_range2 in enumerate(movement_ranges_yaw2):
        print(f"Movement {i+1}: Range = {yaw_range1:.2f} degrees")
    significant_movements2 = [(pair1, mrange1) for pair2, mrange2 in zip(movement_pairs2, movement_ranges_yaw2) if mrange2 >= 0.02]
